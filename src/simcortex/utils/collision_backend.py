...
Important behavior
------------------
- ``FCL_MAX_CONTACTS`` is configured at import time. The canonical environment
  variable is ``SIMCORTEX_FCL_MAX_CONTACTS``; the historical
  ``SCPP_FCL_MAX_CONTACTS`` name remains supported as a compatibility fallback.
...
def _read_fcl_max_contacts(default: int = 2_000_000) -> int:
    """Read the contact limit while preserving the historical environment name."""
    if os.environ.get("SIMCORTEX_FCL_MAX_CONTACTS", "") != "":
...
# saturated and the reported contact counts are lower bounds.
FCL_MAX_CONTACTS = _read_fcl_max_contacts()
