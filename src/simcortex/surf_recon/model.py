...
class MUNet(nn.Module):
    ...
    def __init__(self, C_in=1, C_hid=[8,16,32,32,32,32], K=3):
        ...
        self.conv3 = nn.Conv3d(in_channels=C_hid[1], out_channels=C_hid[2],
                               kernel_size=K, stride=2, padding=1)
        ...
        # Upsampling module (not used in modified forward)
...
class SurfDeform(nn.Module):
    """
    Surface diffeomorphic deformation network for cortical surface reconstruction.
    ...
    def __init__(self, C_in=1,
        ...
        self.register_buffer("scale", torch.tensor(inshape, dtype=torch.float32))
    ...
    def forward(self, vert, vol, n_steps):
    ...
    def interpolate(self, coord, src):
        ...
        """
        for i in range(3):
            s = self.scale[i].to(coord.device)
            coord[..., i] = 2 * coord[..., i] / (s - 1) - 1
        
        
        coord = coord.flip(-1)    
        coord = coord.to(src.device)
        out = F.grid_sample(
