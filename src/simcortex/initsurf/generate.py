...
def _write_json_atomic(path: str | Path, payload: Dict[str, Any]) -> None:
    """Write JSON atomically to avoid publishing a partially written file."""
    out_path = Path(path)
...
def _simcortex_version() -> str:
    """Return the installed SimCortex package version."""
    try:
...
def write_dataset_description(
    ...
    """Write BIDS derivative metadata using the package software version."""
        ...
        "GeneratedBy": [
            {
                ...
                "Description": (
                    "Initial cortical surfaces in MNI152 space generated by SimCortex InitSurf."
                ),
            }
        ],
    }
...
def _validate_params(params: Dict[str, Any]) -> None:
    """Validate InitSurf configuration without changing valid parameters."""
    required = {
        "gap_size",
        "sdf_sigma",
        "topo_threshold",
    ...
    if wm_start < wm_min:
        raise ValueError(
            "params.wm_start_level must be >= params.wm_min_level"
        )
    if wm_inset < 0:
    ...
    if pial_min > pial_max:
        raise ValueError(
            "params.pial_min_level must be <= params.pial_max_level"
        )
    if pial_floor < 0:
        raise ValueError(
            "params.pial_absolute_floor must be >= 0"
        )
...
def _validate_runtime_dependencies() -> None:
    """Validate optional native dependencies before launching worker jobs."""
    try:
        CollisionManager()
    except Exception as exc:
        raise RuntimeError(
            "InitSurf collision checking requires python-fcl. "
            'Install it with: python -m pip install "simcortex[initsurf]"'
        ) from exc

    # Ensure the packaged topology LUT can be loaded before workers are started.
    get_topo_correct()
...
def main(cfg: DictConfig) -> None:
    ...
    if df.empty:
        raise ValueError(
            f"No rows found in split_file for split='{split_name}'"
        )
