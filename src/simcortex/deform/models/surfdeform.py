...
class DualMUNetV2(nn.Module):
    ...
    def __init__(
        ...
        geom_depth: int = 6,              # <=6 (learned depth for geom)
        K: int = 3,
        gn_groups: int = 8,
        gate_init: float = -3.0,
        dropout: float = 0.0,
    ...
    def forward(self, x):
        if x.ndim != 5:
            raise ValueError(
                f"DualMUNetV2 expects a 5D tensor (B,C,D,H,W), got {tuple(x.shape)}"
            )
        if int(x.shape[1]) != self.C_in:
            raise ValueError(
                f"DualMUNetV2 expected {self.C_in} input channels, "
                f"got {int(x.shape[1])}"
            )
...
class SurfDeform(nn.Module):
    ...
    def forward(
        ...
        if int(vol.shape[1]) != self.C_in:
            raise ValueError(
                f"Input vol has {int(vol.shape[1])} channels, expected {self.C_in}"
            )
        if tuple(vol.shape[2:]) != self.inshape:
            raise ValueError(
                f"Input vol shape {tuple(vol.shape[2:])} != inshape {self.inshape}"
            )
